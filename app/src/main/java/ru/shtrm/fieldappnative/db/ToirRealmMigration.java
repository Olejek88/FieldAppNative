...
class ToirRealmMigration implements RealmMigration {
    ...
    private boolean testPropsFields(DynamicRealm realm) {
        ...
        for (RealmObjectSchema realmObject : realmObjects) {
            ...
            try {
                Class<?> c = Class.forName("ru.shtrm.fieldappnative.db.realm." + tableName);
...
//                    propsType.put(prop.getName(), prop.getGenericType().toString());
        ...
        // TODO: реализовать загрузку classes.dex и поиск в нём <Lru.shtrm.fieldappnative.db.realm.*;>
        ...
        try {
            ...
            while (iter.hasMoreElements()) {
                ...
                if (classPath.contains("ru.shtrm.fieldappnative.db.realm") && !classPath.contains("$")) {
                    try {
